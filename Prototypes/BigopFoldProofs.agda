module Prototypes.BigopFoldProofs where
  ...
  module Binomials where
    ...
    module BinomialTheorem where
      ...
      proof : ∀ x n → Σ[ k ← 0 …+ (suc n) $ n choose k * x ^ k ] ≈ (suc x) ^ n
      proof x 0       = refl
      proof x (suc n) = begin
        1 + Σ[ k ← 1 …+ suc n $ (suc n) choose k * x ^ k ]
          ≡⟨ refl {x = 1} ⟨ +-cong ⟩ begin
            Σ[ k ← 1 …+ suc n $ (suc n) choose k * x ^ k ]
              ≡⟨ sym $ Σ-cong {g = f (suc n)} suc (λ _ → refl) (0 …+ suc n)
                       ⟨ trans ⟩ P.cong (fold (f (suc n))) (suc-lemma 0 (suc n)) ⟩
            Σ[ k ← 0 …+ suc n $ (suc n) choose (suc k) * x ^ (suc k) ]
              ≡⟨ Σ-cong′ {f = λ k → f (suc n) (suc k)} (λ _ → refl) (0 …+ suc n) ⟩
            Σ[ k ← 0 …+ suc n $ (n choose k + n choose (suc k)) * x ^ (suc k) ]
              ≡⟨ Σ-cong′ {f = λ k → (n choose k + n choose (suc k)) * x ^ (suc k)}
                         (λ k → distribʳ (x ^ (suc k)) (n choose k) _) (0 …+ suc n) ⟩
            Σ[ k ← 0 …+ suc n $ n choose k * x ^ (suc k)
                                  + n choose (suc k) * x ^ (suc k) ]
              ≡⟨ sym $ Σ-lift {J = ℕ}
                              (λ k → n choose k * x ^ (suc k)) (λ k → f n (suc k))
                              (0 …+ suc n) ⟩
            Σ[ k ← 0 …+ suc n $ n choose k * x ^ (suc k) ]
            + Σ[ k ← 0 …+ suc n $ n choose (suc k) * x ^ (suc k) ] ∎
          ⟩
        1 + (Σ[ k ← 0 …+ suc n $ n choose k * x ^ (suc k) ]
             + Σ[ k ← 0 …+ suc n $ n choose (suc k) * x ^ (suc k) ])
          ≡⟨ +-reorder 1 Σ[ k ← 0 …+ suc n $ n choose k * x ^ (suc k) ] _ ⟩
        Σ[ k ← 0 …+ suc n $ n choose k * x ^ (suc k) ]
        + (1 + Σ[ k ← 0 …+ suc n $ n choose (suc k) * x ^ (suc k) ])
          ≡⟨ ➀ ⟨ +-cong ⟩ ➁ ⟩
          x * Σ[ k ← 0 …+ suc n $ n choose k * x ^ k ]
        +     Σ[ k ← 0 …+ suc n $ n choose k * x ^ k ]
          ≡⟨ +-cong (refl {x = x * _})
                    (sym $ proj₁ *-identity Σ[ k ← 0 …+ suc n $ f n k ]) ⟩
          x * Σ[ k ← 0 …+ suc n $ n choose k * x ^ k ]
        + 1 * Σ[ k ← 0 …+ suc n $ n choose k * x ^ k ]
          ≡⟨ sym $ distribʳ _ x 1 ⟩
        (x + 1) * Σ[ k ← 0 …+ suc n $ n choose k * x ^ k ]
        ...
        where
          ...
          ➀ = begin
            Σ[ k ← 0 …+ suc n $ n choose k * x ^ (suc k) ]
              ≡⟨ Σ-cong′ reorder (0 …+ suc n) ⟩
            Σ[ k ← 0 …+ suc n $ x * (n choose k * x ^ k) ]
              ≡⟨ sym $ Σ-distrˡ (λ k → n choose k * x ^ k) x (0 …+ suc n) ⟩
            x * Σ[ k ← 0 …+ suc n $ n choose k * x ^ k ] ∎
          ...
          ➁ = begin
            1 + Σ[ k ← 0 …+ suc n $ n choose (suc k) * x ^ (suc k) ]
              ≡⟨ (refl {x = 1}) ⟨ +-cong ⟩ begin
                Σ[ k ← 0 …+ suc n $ n choose (suc k) * x ^ (suc k) ]
                  ≡⟨ Σ-cong {g = f n} suc (λ k → refl) (0 …+ suc n) ⟩
                Σ[ k ← map suc (0 …+ suc n) $ n choose k * x ^ k ]
