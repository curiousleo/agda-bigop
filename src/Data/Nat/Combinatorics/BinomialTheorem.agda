...
split : ∀ x n →
  Σ[ k ← 1 … suc n ] (suc n) choose k * x ^ k ≡
    Σ[ k ← 0 … n ] n choose k * x ^ (suc k) + Σ[ k ← 0 … n ] n choose (suc k) * x ^ (suc k)
split x n =
  begin
    Σ[ k ← 1 … suc n ] (suc n) choose k * x ^ k
    ...
    Σ[ k ← 0 … n ] (n choose k + n choose (suc k)) * x ^ (suc k)
       ≡⟨ Σ.cong {f = λ k → (n choose k + n choose (suc k)) * x ^ (suc k)}
               (0 … n) P.refl (λ k → distribʳ (x ^ (suc k)) (n choose k) _) ⟩
    Σ[ k ← 0 … n ] (n choose k * x ^ (suc k) + n choose (suc k) * x ^ (suc k))
       ≡⟨ sym $ Σ.∙-distr (λ k → n choose k * x ^ (suc k)) (λ k → f x n (suc k)) (0 … n) ⟩
    Σ[ k ← 0 … n ] n choose k * x ^ (suc k) + Σ[ k ← 0 … n ] n choose (suc k) * x ^ (suc k)
  ∎
...
choose-suc x n  =
  begin
    ...
    Σ[ k ← (1 … n) ∷ʳ (suc n) ] n choose k * x ^ k
      ≡⟨ Σ.snoc (f x n) (suc n) (1 … n) ⟩
    Σ[ k ← 1 … n ] n choose k * x ^ k + n choose (suc n) * x ^ (suc n)
      ≡⟨ +-cong (refl {x = Σ[ k ← 1 … n ] f x n k}) (choose-+ 0 n ⟨ *-cong ⟩ refl ⟨ trans ⟩ zeroˡ n) ⟩
...
binomial-theorem : ∀ x n → Σ[ k ← 0 … n ] n choose k * x ^ k ≡ (suc x) ^ n
binomial-theorem x zero    = refl
binomial-theorem x (suc n) =
  begin
    1 + Σ[ k ← 1 … suc n ] (suc n) choose k * x ^ k
      ≡⟨ refl {x = 1} ⟨ +-cong ⟩ (split x n) ⟩
    1 + (Σ[ k ← 0 … n ] n choose k * x ^ (suc k) + Σ[ k ← 0 … n ] n choose (suc k) * x ^ (suc k))
      ≡⟨ +-reorder 1 (Σ[ k ← 0 … n ] n choose k * x ^ (suc k)) _ ⟩
    Σ[ k ← 0 … n ] n choose k * x ^ (suc k) + (1 + Σ[ k ← 0 … n ] n choose (suc k) * x ^ (suc k))
      ≡⟨ left-distr x n ⟨ +-cong ⟩ shift x n ⟩
    x * (Σ[ k ← 0 … n ] n choose k * x ^ k) + Σ[ k ← 0 … n ] n choose k * x ^ k
      ≡⟨ refl {x = x * _} ⟨ +-cong ⟩ sym (proj₁ *-identity _) ⟩
    x * (Σ[ k ← 0 … n ] n choose k * x ^ k) + 1 * (Σ[ k ← 0 … n ] n choose k * x ^ k)
      ≡⟨ sym $ distribʳ _ x 1 ⟩
